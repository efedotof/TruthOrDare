...
class _page2_1State extends State<page2_1> {
 final CardSwiperController controller = CardSwiperController();
   ...
   void rigth_state() async{
     setState(() {
      if(crug < 2){
        ...
        if(isencition != array_vopros_destvie2_1.length){
          state_0();
          // rand_numbers2();
          int cacheStat = list2[isencition];
          cards.clear();
          count += 1;
          cards.add(cards_widgets(player:player_number,name:array_vopros_destvie2_1[cacheStat],));
          state_0();
  ...
  void left_state(){
    setState(() {
      if(crug < 2){
        ...
        if(isencition != array_vopros2_1.length){
        cards.clear();
        // rand_numbers();
        int cacheStat = list[isencition];
        count += 1;
        cards.add(cards_widgets(player: player_number,name: array_vopros2_1[cacheStat],));
        state_0();
        cards.add(cards_widgets(player: player_number,name: "Правда или Действие ?",));
        isencition++;
      }else{
  ...
  void state_0(){
   ...
   void rand_numbers (){
    var listim = List.generate(array_vopros_destvie2_1.length, (int i) => i);                                                                                     
    listim.shuffle();       
  ...
  void rand_numbers2 (){
    var listim = List.generate(array_vopros2_1.length, (int i) => i);                                                                                     
    listim.shuffle();       
    for(int i =0; i < listim.length;i++){
      list.add(listim[i]);
    }                          
