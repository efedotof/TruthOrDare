...
class page2_2 extends StatefulWidget {
  const page2_2({super.key});

  @override
  State<page2_2> createState() => _page2_2State();
}
...
class _page2_2State extends State<page2_2> {
 final CardSwiperController controller = CardSwiperController();
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Text('К сожалению было добавлено максимум игроков'),
            actions: <Widget>[
              TextButton(
                onPressed: () {
                  Navigator.pop(context);
  ...
  Future <void> _displey_text_dialog(BuildContext context){
    ...
    void adds(){
    print(player_naming_start_game.length);
    setState(() {
      if(player_numbers > 5){
      _showMaterialDialog();
     }else{
...
, icon: Icon(Icons.arrow_back_ios)),
              Column(
                children: [
                  Text("Для Компании".toUpperCase(),style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold),),
                ],
   ...
   void rigth_state() async{
     setState(() {
      if(crug < 2){
        ...
        if(isencition != array_vopros_destvie2_2.length){
          state_0();
          // rand_numbers2();
          int cacheStat = list2[isencition];
          cards.clear();
          count += 1;
          cards.add(cards_widgets(player:player_number,name:array_vopros_destvie2_2[cacheStat],));
          state_0();
  ...
  void left_state(){
    setState(() {
      if(crug < 2){
        ...
        if(isencition != array_vopros2_2.length){
        cards.clear();
        // rand_numbers();
        int cacheStat = list[isencition];
        count += 1;
        cards.add(cards_widgets(player: player_number,name: array_vopros2_2[cacheStat],));
        state_0();
        cards.add(cards_widgets(player: player_number,name: "Правда или Действие ?",));
        isencition++;
      }else{
  ...
  void rand_numbers (){
    var listim = List.generate(array_vopros_destvie2_2.length, (int i) => i);                                                                                     
    listim.shuffle();       
  ...
  void rand_numbers2 (){
    var listim = List.generate(array_vopros2_2.length, (int i) => i);                                                                                     
    listim.shuffle();       
    for(int i =0; i < listim.length;i++){
      list.add(listim[i]);
    }                          
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Column(
                  children: [
                    Text("Средняя".toUpperCase(),style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold),),
                  ],
