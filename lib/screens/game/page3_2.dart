...
class _page3_2State extends State<page3_2> {
   ...
   void rigth_state() async{
     setState(() {
      if(crug < 2){
        ...
        if(isencition != array_vopros_destvie3_2.length){
          state_0();
          // rand_numbers2();
          int cacheStat = list2[isencition];
          cards.clear();
          count += 1;
          cards.add(cards_widgets(player:player_number,name:array_vopros_destvie3_2[cacheStat],));
          state_0();
  ...
  void left_state(){
    setState(() {
      if(crug < 2){
        ...
        if(isencition != array_vopros3_2.length){
        cards.clear();
        // rand_numbers();
        int cacheStat = list[isencition];
        count += 1;
        cards.add(cards_widgets(player: player_number,name: array_vopros3_2[cacheStat],));
        state_0();
        cards.add(cards_widgets(player: player_number,name: "Правда или Действие ?",));
        isencition++;
      }else{
  ...
  void rand_numbers (){
    var listim = List.generate(array_vopros_destvie3_2.length, (int i) => i);                                                                                     
    listim.shuffle();       
  ...
  void rand_numbers2 (){
    var listim = List.generate(array_vopros3_2.length, (int i) => i);                                                                                     
    listim.shuffle();       
    for(int i =0; i < listim.length;i++){
      list.add(listim[i]);
    }                          
  ...
  void spravs(){
    setState(() {
          Navigator.push(
    context,
    MaterialPageRoute(builder: (context) => const sprav()),);
      });
  }
  ...
  Future <void> _displey_text_dialog(BuildContext context){
    return showDialog(context: context, builder:
      (context){
        return AlertDialog(title: const Text("Введите имя для игрока"),
      ...
      child: Column(
        ...
        children: [
           Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              IconButton(onPressed:   exits, icon: const Icon(Icons.arrow_back_ios)),
              Column(
                children: [
